...
namespace AHT\ProductTags\Block\Index;
...
class Index extends \Magento\Catalog\Block\Product\AbstractProduct
{
    ...
    protected $_request;
    /**
     * @var \AHT\ProductTags\Model\TagsFactory
     */
     ...
     * @param \AHT\ProductTags\Model\TagsFactory $tagsFactory
    ...
    protected function _prepareLayout()
    {
        ...
        if($this->_request->getParam('tag')){
            $title = __('Product Tags %1', $this->_request->getParam('tag'));
        ...
        if ($this->getTaggedCollection()) {
            $pager = $this->getLayout()
                ->createBlock(
                    'AHT\ProductTags\Block\CustomPager',
                    'aht.tags.pager'
    ...
    public function getAllTags(){
        $collection = $this->_tagsFactory->getCollection()->addFieldToFilter('status',1);;
